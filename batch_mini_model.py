...
input_sentences = [
    ...
    "I saw the elephant with " + tokenizer.mask_token + " on my vacation and it was really cool",
    "The band played music for the dancers on the " + tokenizer.mask_token + " during the game"
]

distance_files = [
    'data/distance_Jane.pkl',
    'data/distance_elephant.pkl',
    'data/distance_band.pkl'
]

dump_files = [
    'data/sentence_pickles/Jane_first.pkl',
    'data/sentence_pickles/elephant_first.pkl',
    'data/sentence_pickles/band_first.pkl'
]

for i in range(len(input_sentences)):
    print("sentence: ", input_sentences[i])
    # obtaining distance matrix 
    with open(distance_files[i], 'rb') as f:
        distance_matrices = pickle.load(f)

    # tokenizing the input sentence
    input = tokenizer(input_sentences[i], return_tensors = "pt")
    ...
    updated_hidden_first = torch.transpose(torch.matmul(torch.linalg.inv(b_matrix), torch.transpose(new_hidden_first, 0, 1)), 1, 0)
    ...
    for token in top_10_first_context:
        ...
        new_sentence = input_sentences[i].replace(tokenizer.mask_token, word)
        print(new_sentence)


    # Create a file with 'filename'.pkl in the respective folder, then run the following two commands
    with open(dump_files[i], 'wb') as f:  # Python 3: open(..., 'wb')
