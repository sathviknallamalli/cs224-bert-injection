...
submodel = BertForMaskedLM.from_pretrained('bert-base-cased', output_hidden_states=True, return_dict = True, num_hidden_layers = 4)


TARGET_SENTENCE = input_sentences[22]

distance_first_context = torch.from_numpy(distance_dict[TARGET_SENTENCE][0])
...
for layerCount in range(4, 10):
    print('intervening after layer ', layerCount)
    ...
    for i in range(layerCount, layerCount+1):
    ...
    hidden_states = output_first_context.hidden_states[-1] 
