...
for i in range(1,2):
    print(i)
    print("current sentence: ", TARGET_SENTENCE)
    ...
    import_b = torch.load('bert-base-distance-probes/ptb-prd-BERTbase-rank4.params', map_location='cpu')
    ...
    distance_first_context = torch.from_numpy(distance_dict[TARGET_SENTENCE][i])
    ...
    while itercount<500:
        ...
        if itercount % 100 == 0:
            ...
            print('top 10 words at this point ', itercount)
            ...
            for k in range(10):
                report.write(f"{tokenizer.decode([top_10_first_context[k]])} {top_10_prob_first_context[k].item()} \n")
                print(tokenizer.decode([top_10_first_context[k]]), top_10_prob_first_context[k].item())
